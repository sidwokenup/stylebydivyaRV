...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "CATEGORY", href: "#", isDropdown: true, items: [
      { name: "Tops", href: "/category/tops" },
      { name: "Skirt", href: "/category/skirt" },
      { name: "Trouser", href: "/category/trouser" },
      { name: "Outfits", href: "/category/outfits" },
      { name: "Couple Combo", href: "/category/couple-combo" },
    ]},
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navLinks.map((link) => (
                <div 
                  ...
                  onMouseEnter={() => link.isDropdown && setHoveredLink(link.name)}
                  onMouseLeave={() => link.isDropdown && setHoveredLink(null)}
                >
                  <Link
                    href={link.href}
                    className="hover:text-[#D4AF37] px-3 py-2 rounded-md text-sm font-medium tracking-wider transition-colors duration-300 flex items-center gap-1"
                  >
                    {link.name}
                    {link.isDropdown && (
                    ...
                    <AnimatePresence>
                      {hoveredLink === link.name && (
                        <motion.div
                          ...
                          className="absolute left-0 mt-0 w-48 bg-white text-black shadow-lg rounded-sm py-2 border border-gray-100 z-50"
                        >
                          {link.items?.map((item) => (
                            <Link
                              key={item.name}
                              href={item.href}
                              className="block px-4 py-2 text-sm hover:bg-gray-50 hover:text-[#D4AF37] transition-colors"
                            >
                              {item.name}
                            </Link>
                          ))}
                  ...
                  <div key={link.name}>
                    {link.isDropdown ? (
                      <div className="flex flex-col">
                        <button
                          className="text-2xl font-serif tracking-widest text-white hover:text-[#D4AF37] transition-colors duration-300 border-b border-white/10 pb-4 text-left flex justify-between items-center w-full"
                          onClick={() => setHoveredLink(hoveredLink === link.name ? null : link.name)}
                        >
                          {link.name}
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-5 h-5 transition-transform ${hoveredLink === link.name ? 'rotate-180' : ''}`}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                          </svg>
                        </button>
                        ...
                        <AnimatePresence>
                          {hoveredLink === link.name && (
                            <motion.div
                              ...
                              className="flex flex-col pl-4 mt-2 gap-3 overflow-hidden"
                            >
                              {link.items?.map((item) => (
                                <Link
                                  key={item.name}
                                  href={item.href}
                                  onClick={toggleMobileMenu}
                                  className="text-lg text-gray-400 hover:text-[#D4AF37] tracking-wide transition-colors py-1"
                                >
                                  {item.name}
                                </Link>
                              ))}
                      ...
                      <Link
                        href={link.href}
                        onClick={toggleMobileMenu}
                        className="text-2xl font-serif tracking-widest text-white hover:text-[#D4AF37] transition-colors duration-300 border-b border-white/10 pb-4 block"
                      >
                        {link.name}
                      </Link>
                    )}
